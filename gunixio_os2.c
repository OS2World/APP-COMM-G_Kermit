...
/*
  ...
  This copy contains changes from Dallas E. Legan <dallasii@surfree.com>
...
extern int text  ;                      /*  text mode    d.e.l.   */
...
zopeni(name) char *name; {		/* Open existing file for input */
    /*  original:  ifp = fopen(name,"r");      d.e.l.  */
    ifp = fopen(name,"rb");
...
zopeno(name) char *name; {		/* Open new file for output */
    errno = 0;
    /*  ofp = fopen(name,"w");       d.e.l.  */
    /*  ofp = fopen(name,"wb");    1rst try  d.e.l.  */
    ofp = fopen(name,  ( text ? "w" : "wb" ) )  ;
...
zbackup(fn) char * fn; {		/* Back up existing file */
	...
	if (stat(newname,&buf) < 0) {	/* If it doesn't exist */
	    ...
	    /*  if (link(fn,newname) <  0) { /  * Rename old file to backup name */
                                         /*  d.e.l.   */
	    if (rename (fn,newname) <  0) { /* Rename old file to backup name */
		if (debug)
